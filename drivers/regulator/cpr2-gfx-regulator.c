		      ...
		      int vdd_mx_vmin)
{
	...
	if (cpr_vreg->pd_mx == NULL)
		return 0;

	rc = dev_pm_genpd_set_performance_state(cpr_vreg->pd_mx, vdd_mx_vmin);
	cpr_debug(cpr_vreg, "[corner:%d] %d uV\n", corner, vdd_mx_vmin);

	if (!rc)
		cpr_vreg->pd_mx_vmin = vdd_mx_vmin;
	else
					...
					enum voltage_change_dir dir)
{
	...
	if (dir != NO_CHANGE && cpr_vreg->pd_mx != NULL)
		vdd_mx_vmin = cpr_vreg->pd_mx_corner_map[corner];
	...
	if (!rc && vdd_mx_vmin && dir == UP) {
		if (vdd_mx_vmin != cpr_vreg->pd_mx_vmin)
...
static int cpr2_gfx_regulator_enable(struct regulator_dev *rdev)
{
	...
	if (cpr_vreg->pd_mx) {
		dev_pm_genpd_set_performance_state(cpr_vreg->pd_mx, 384);
		rc = pm_runtime_get_sync(cpr_vreg->pd_mx);
		pm_runtime_forbid(cpr_vreg->pd_mx);
		if (rc) {
			cpr_err(cpr_vreg, "enable: vdd_mx: rc=%d\n", rc);
			mutex_unlock(&cpr_vreg->cpr_mutex);
			return rc;
		}
	}
...
static int cpr2_gfx_regulator_disable(struct regulator_dev *rdev)
{
	...
	mutex_lock(&cpr_vreg->cpr_mutex);
pr_err("cpr2 regulator disable\n");
	rc = regulator_disable(cpr_vreg->vdd_gfx);
	if (!rc) {
		if (cpr_vreg->pd_mx) {
			rc = dev_pm_genpd_set_performance_state(
							cpr_vreg->pd_mx, 0);
			if (rc) {
				cpr_err(cpr_vreg, "disable: vdd_mx: rc=%d\n",
					rc);
				goto _exit;
			}
			pm_runtime_put(cpr_vreg->pd_mx);
...
static int cpr_parse_vdd_mx_parameters(struct cpr2_gfx_regulator *cpr_vreg)
{
	...
	if (!of_find_property(of_node, "qcom,mx-corner-map", &len)) {
		cpr_err(cpr_vreg, "qcom,mx-corner-map missing");
		return -EINVAL;
	}
	...
	if (size != cpr_vreg->num_corners) {
		cpr_err(cpr_vreg,
			"qcom,mx-corner-map length=%d is invalid: required:%u\n",
			size, cpr_vreg->num_corners);
		return -EINVAL;
	}

	cpr_vreg->pd_mx_corner_map = devm_kzalloc(cpr_vreg->dev,
		(size + 1) * sizeof(*cpr_vreg->pd_mx_corner_map),
			GFP_KERNEL);
	if (!cpr_vreg->pd_mx_corner_map) {
		cpr_err(cpr_vreg,
			"Can't allocate memory for pd_mx_corner_map\n");
		return -ENOMEM;
	}

	rc = of_property_read_u32_array(of_node,
				"qcom,mx-corner-map",
				&cpr_vreg->pd_mx_corner_map[1],
				cpr_vreg->num_corners);
	if (rc)
		cpr_err(cpr_vreg,
			"read qcom,mx-corner-map failed, rc = %d\n", rc);
...
static int cpr_gfx_init(struct cpr2_gfx_regulator *cpr_vreg)
{
	...
	/* Optional */
	cpr_vreg->pd_mx = dev_pm_domain_attach_by_name(cpr_vreg->dev, "mx");
	if (IS_ERR(cpr_vreg->pd_mx)) {
		if (PTR_ERR(cpr_vreg->pd_mx) == -EPROBE_DEFER) {
			return -EPROBE_DEFER;
		}
	...
	if (rc) {
		cpr_err(cpr_vreg, "parsing genpd mx parameters failed: %d\n",
			rc);
		return rc;
	}
	/* Set a min corner to kickstart the MX before vdd_gfx */
	cpr_vreg->pd_mx_vmin = cpr_vreg->pd_mx_corner_map[1];
...
static void cpr_gfx_exit(struct cpr2_gfx_regulator *cpr_vreg)
{
	...
	if (cpr_vreg->pd_mx)
		dev_pm_genpd_set_performance_state(cpr_vreg->pd_mx, 0);
}
