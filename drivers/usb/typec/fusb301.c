				...
				enum power_supply_type type)
{
	...
	if (IS_ERR_VALUE((unsigned long)rc)) {
		...
		if (IS_ERR_VALUE((unsigned long)fusb301_reset_device(chip)))
...
static void fusb301_src_detected(struct fusb301_chip *chip)
{
	...
	chip->type_c_psy_d.type = POWER_SUPPLY_TYPE_USB_TYPE_C;
}
...
static void fusb301_snk_detected(struct fusb301_chip *chip)
{
	...
	if ((!chip->triedsnk) &&
		...
		if (IS_ERR_VALUE((unsigned long)fusb301_set_mode(chip, FUSB301_SNK)) ||
			IS_ERR_VALUE((unsigned long)fusb301_set_chip_state(chip,
						FUSB_STATE_UNATTACHED_SNK))) {
...
static void fusb301_timer_try_expired(struct fusb301_chip *chip)
{
	...
	if (IS_ERR_VALUE((unsigned long)fusb301_set_mode(chip, FUSB301_SRC)) ||
		IS_ERR_VALUE((unsigned long)fusb301_set_chip_state(chip,
					FUSB_STATE_UNATTACHED_SRC))) {
...
*/
	...
	if ((chip->triedsnk && chip->pdata->try_snk_emulation)
		...
		if (IS_ERR_VALUE((unsigned long)fusb301_set_mode(chip,
...
static void fusb301_timer_work_handler(struct work_struct *work)
{
	...
	if (chip->state == FUSB_STATE_TRY_SNK) {
		if (fusb301_is_vbus_on(chip)) {
			if (IS_ERR_VALUE((unsigned long)fusb301_set_mode(chip,	chip->pdata->init_mode))) {
				dev_err(cdev, "%s: failed to set init mode\n", __func__);
			}
			chip->triedsnk = !chip->pdata->try_snk_emulation;
			mutex_unlock(&chip->mlock);
...
work_unlock:
	...
	if (int_sts && chip->dwork_looped++ < DWORK_LOOP_MAX) {
		__pm_wakeup_event(&chip->wlock, FUSB301_WAKE_LOCK_TIMEOUT);
...
static int fusb301_init_gpio(struct fusb301_chip *chip)
{
	...
	chip->pinctrl = devm_pinctrl_get(cdev);
	if (IS_ERR(chip->pinctrl)) {
		dev_err(cdev, "Cannot get pinctrl\n");
		return PTR_ERR(chip->pinctrl);
	}

	chip->pinstate_default = pinctrl_lookup_state(chip->pinctrl,
						      PINCTRL_STATE_DEFAULT);
	if (IS_ERR(chip->pinstate_default)) {
		dev_err(cdev, "Cannot find default pinctrl state\n");
		return -ENODEV;
	}
	rc = pinctrl_select_state(chip->pinctrl, chip->pinstate_default);
	if (rc) {
		dev_err(cdev, "Cannot select pinctrl state\n");
		return rc;
	}
			...
			const struct i2c_device_id *id)
{
	...
	wakeup_source_create("fusb301_wake");
	//
...
err3:
	...
	wakeup_source_destroy(&chip->wlock);
